...
int rblt() {
     while(!((*READ_IO(UART_BASE + 20) & 0x00000001)==0x00000001));
        return (uint32_t)*READ_IO(UART_BASE + 0);
}
...
{
	pmd_helper(0xD4,speed);
}

void backward(int speed)
{
	pmd_helper(0x2B,speed);
}

void turn_left(int speed)
{
    pmd_helper(0x66,speed);
}

void turn_right(int speed)
{
	pmd_helper(0x99,speed);
}
...
{//corrected by xiaohan: this is actually not serial interrupt handler!
 ...
 //otherwise the OS will fall into the dead loop of dealing with "previous" EIC interrupt.   
	...
	if (st == 0) {
		...
		turn_right(15);
		int cnt;
		for (cnt = 0; cnt < 24000; cnt++)
			delay();
		*WRITE_IO(WHEEL_BASE)=0;
		eic_ack(0);
	}
	else if (st == 1) {
		kprintf("Bluetooth!\n\r");
